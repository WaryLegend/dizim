...
// Khởi tạo SDK bằng Client mới đồng bộ trên toàn hệ thống Backend
...
export default {
  async handleChat(ctx) {
    try {
      ...
      console.log('Step 2: Truy vấn dữ liệu MySQL, mode =', mode);
      ...
      for (const row of records) {
        ...
        // Đảm bảo dữ liệu mảng số thực được parse chuẩn xác
        const docVector = typeof row.embedding === 'string' ? JSON.parse(row.embedding) : row.embedding;
        
        // Kiểm tra phòng ngừa lệch kích thước vector giữa các phiên bản model cũ/mới
        if (userVector.length !== docVector.length) {
          strapi.log.warn(`[RAG Warning] Lệch kích thước Vector tại bản ghi ID ${row.id}. Vui lòng re-upload file để chạy lại lifecycle.`);
          continue;
        }
      ...
      // Gộp nội dung các đoạn tài liệu được bóc tách từ file làm ngữ cảnh (Context)
      ...
      const finalSystemPrompt = `
        ...
        [CONTEXT / TÀI LIỆU TRÍCH XUẤT TỪ FILE HỆ THỐNG]
      ...
      // Generate response từ Gemini 2.5 Flash với cấu hình mới nhất
